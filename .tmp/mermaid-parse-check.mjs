...
var autoLayoutOrigin = { x: 120, y: 120 };
var autoLayoutSpacing = {
  rank: 160,
  node: 72
};
var flowchartShapeAliases = {
  rectangle: "rectangle",
  rect: "rectangle",
  process: "rectangle",
  proc: "rectangle",
...
function formatExpandedFlowchartNode(mermaidShape) {
  return (id, label) => `  ${id}@{ shape: ${mermaidShape}, label: "${label}" }`;
}
var flowchartNodeFormatters = {
...
var sequenceArrowByMessageType = {
  sync: "->",
  async: "->>",
  dashed: "-->",
  dashedAsync: "-->>"
};
var sequenceMessageTypeByArrow = Object.fromEntries(
  Object.entries(sequenceArrowByMessageType).map(([messageType, arrow]) => [arrow, messageType])
);
var erCardinalityMarkers = {
  source: {
    one: "||",
    zeroOrOne: "o|",
    oneOrMore: "}|",
...
function toMermaid(nodes, edges, direction, type) {
  switch (type) {
    case "sequence":
...
function toFlowchart(nodes, edges, direction) {
  const nodeLines = nodes.map((node) => formatFlowchartNode(sanitizeId(node.id), escapeLabel(node.data.label || node.id), node.data.shape)).join("\n");
  ...
  const nodeStyleLines = nodes.map((node) => formatFlowchartNodeStyle(sanitizeId(node.id), node.data.style)).filter(Boolean).join("\n");
  const edgeStyleLines = edges.map((edge, index) => formatFlowchartEdgeStyle(index, edge.data?.visualStyle)).filter(Boolean).join("\n");
  return [`flowchart ${direction}`, nodeLines, edgeLines, nodeStyleLines, edgeStyleLines].filter(Boolean).join("\n");
}
function formatFlowchartNode(id, label, shape = "rectangle") {
  return flowchartNodeFormatters[shape](id, label);
}
function formatFlowchartLink(lineStyle = "arrow", label) {
  return flowchartLinkFormatters[lineStyle](label);
}
function formatFlowchartNodeStyle(id, style) {
  const declarations = [
...
function formatFlowchartEdgeStyle(index, style) {
  const declarations = [
    style?.strokeColor ? `stroke:${style.strokeColor}` : "",
...
function toSequenceDiagram(nodes, edges) {
  ...
  const participantLines = orderedNodes.map((node) => formatSequenceParticipantDeclaration(node)).join("\n");
  const messageLines = edges.map((edge, index) => ({ edge, index })).sort((firstItem, secondItem) => compareSequenceEdges(firstItem.edge, secondItem.edge, firstItem.index, secondItem.index)).map(({ edge }) => {
...
${memberLines}
  ...
  const relationshipLines = edges.map((edge) => {
...
function toStateDiagram(nodes, edges) {
  const stateLines = nodes.map((node) => {
  ...
  const transitionLines = edges.map((edge) => {
...
function toErDiagram(nodes, edges) {
...
function toMindmap(nodes, edges) {
...
function toClassId(node) {
  return sanitizeId(node.data.label || node.id);
}
function toErId(node) {
  return sanitizeErId(node.data.label || node.id);
}
function formatIndentedLines(text) {
  return (text ?? "").split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
}
function formatErAttributeLines(text) {
...
function formatErRelationship(edge) {
...
function formatErCardinality(cardinality, side) {
  return erCardinalityMarkers[side][cardinality];
}
function escapeLabel(label) {
  return label.replace(/"/g, '\\"');
}
function escapeSequenceText(text) {
  return normalizeInlineText(text);
}
function formatSequenceArrow(messageType = "async") {
  return sequenceArrowByMessageType[messageType];
}
function formatSequenceParticipantDeclaration(node) {
...
function parseSequenceArrow(operator) {
  return sequenceMessageTypeByArrow[operator] ?? "async";
}
function parseSequenceParticipantDeclaration(line) {
...
function parseSequenceParticipantConfig(rawConfig) {
  if (!rawConfig) {
    return void 0;
  }
  try {
    const parsed = JSON.parse(rawConfig);
    const alias = typeof parsed.alias === "string" ? parsed.alias : void 0;
    const type = isSequenceParticipantType(parsed.type) ? parsed.type : void 0;
    return alias || type ? { alias, type } : void 0;
  } catch {
    return void 0;
  }
}
function isSequenceParticipantType(value) {
  return value === "boundary" || value === "control" || value === "entity" || value === "database" || value === "collections" || value === "queue";
}
function parseSequenceParticipantRef(reference) {
  return reference.replace(/^[+-]+|[+-]+$/g, "");
}
function compareSequenceEdges(firstEdge, secondEdge, firstIndex, secondIndex) {
...
function escapeQuotedText(text) {
  return text.replace(/"/g, '\\"').replace(/\s+/g, " ").trim();
}
function escapeClassText(text) {
  return normalizeInlineText(text);
}
function escapeStateText(text) {
  return normalizeInlineText(text);
}
function sanitizeErId(id) {
  return sanitizeId(id).toUpperCase();
}
function escapeErText(text) {
  return escapeQuotedText(text);
}
function sanitizeErRole(text) {
...
function parseFlowchart(lines) {
  ...
  const direction = (rawDirection === "TB" ? "TD" : rawDirection) ?? "TD";
  ...
  for (const rawLine of lines.slice(1)) {
    ...
    if (line === "end" || line.startsWith("subgraph ") || line.startsWith("classDef ") || line.startsWith("class ") || line.startsWith("click ")) {
      continue;
    }
...
function tryParseFlowchartNode(line, nodes) {
...
function tryParseFlowchartEdge(line, nodes, edges, edgeIndexById, linkIndex) {
...
function applyFlowchartNodeStyle(line, nodes) {
...
function applyFlowchartEdgeStyle(line, edges, edgeIndexById) {
  const styleMatch = line.match(/^linkStyle\s+([^\s]+)\s+(.+)$/);
  if (!styleMatch) {
    return;
  }
  const [, selectorText, declarationText] = styleMatch;
  const targetEdgeIds = selectorText.toLowerCase() === "default" ? edges.map((edge) => edge.id) : selectorText.split(",").map((item) => edgeIndexById.get(Number(item.trim()))).filter((edgeId) => Boolean(edgeId));
...
function parseStyleDeclarations(text) {
  return text.split(",").reduce((result, declaration) => {
...
function parseFlowchartNodeDescriptor(descriptorText) {
...
function parseFlowchartEdgeExpression(line) {
  const pipeLabelMatch = line.match(/^(.*?)\s+(<-->|o--o|x--x|-->|---|-\.->|-\.\-|==>|===|--o|--x)\|([^|]+)\|\s+(.*?)$/);
  if (pipeLabelMatch) {
    const [, sourceText2, linkToken2, label, targetText2] = pipeLabelMatch;
    return createFlowchartEdgeDescriptor(sourceText2, targetText2, parseFlowchartLineStyle(linkToken2), normalizeEscapedText(label));
  }
  const spacedLabelMatch = line.match(/^(.*?)\s+(--|-\.|==)\s+(.+?)\s+(-->|---|\.->|\.\-|==>|==)\s+(.*?)$/);
  if (spacedLabelMatch) {
    const [, sourceText2, startToken, label, endToken, targetText2] = spacedLabelMatch;
    const lineStyle = parseFlowchartLineStylePair(startToken, endToken);
    if (!lineStyle) {
      return void 0;
    }
    return createFlowchartEdgeDescriptor(sourceText2, targetText2, lineStyle, normalizeEscapedText(label));
  }
...
function parseFlowchartEdgeExpressions(line) {
  ...
  return singleEdge ? [singleEdge] : void 0;
}
function parseChainedFlowchartEdgeExpressions(line) {
  ...
  for (let index = 0; index < links.length; index += 1) {
    ...
    const targetText = line.slice(currentLink.index + currentLink.raw.length, nextLink?.index ?? line.length).trim();
...
function createFlowchartEdgeDescriptor(sourceText, targetText, lineStyle, label) {
...
function ensureNodeFromFlowchartDescriptor(nodes, descriptor) {
  return ensureNode(nodes, descriptor.id, descriptor.label, {
    ...descriptor.shape ? { shape: descriptor.shape } : {}
  });
}
function parseFlowchartLineStylePair(startToken, endToken) {
...
function parseFlowchartLineStyle(token) {
  switch (token) {
    case "<-->":
...
function stripLeadingMermaidFrontmatter(lines) {
  if (lines[0]?.trim() !== "---") {
    return lines;
  }
...
function parseErRelationshipData(relationship) {
...
function parseErCardinality(marker, side) {
  if (side === "source") {
    switch (marker) {
      case "||":
        return "one";
...
function ensureNode(nodes, id, label, data) {
...
function createParsedNode(id, label, data) {
  return {
    id,
    type: "editableNode",
    position: { x: 0, y: 0 },
...
function layoutParsedNodes(nodes, edges, direction) {
  return autoLayoutNodes(nodes, edges, direction);
}
function autoLayoutNodes(nodes, edges, direction) {
  ...
  while (queue.length > 0) {
    ...
    for (const targetId of outgoing.get(currentId) ?? []) {
      ...
      if (previousLevel === void 0 || previousLevel < nextLevel) {
...
function layoutSequenceNodes(nodes) {
...
function getSequenceLifelineHeight(messageCount) {
  return Math.max(180, 96 + messageCount * 56);
}
function getAutoLayoutNodeSize(node) {
  const textSections = [
    node.data.label,
...
function layoutParsedNodesLinearly(nodes, direction) {
...
function parseExpandedFlowchartNodeDescriptor(descriptorText) {
  ...
  const shapeToken = properties.shape ? normalizeEscapedText(properties.shape).toLowerCase() : void 0;
  const shape = shapeToken ? flowchartShapeAliases[shapeToken] : void 0;
...
function parseMermaidPropertyMap(text) {
  return splitMermaidProperties(text).reduce((result, entry) => {
...
function splitMermaidProperties(text) {
  const entries = [];
  let current = "";
  let quote;
...
function parseFlowchartNodeGroup(text) {
  const descriptors = splitTopLevelFlowchartGroup(text).map((part) => parseFlowchartNodeDescriptor(part.trim()));
  if (descriptors.some((descriptor) => !descriptor)) {
    return void 0;
  }
  return descriptors;
}
function splitTopLevelFlowchartGroup(text) {
  const parts = [];
  let current = "";
  let roundDepth = 0;
  let squareDepth = 0;
...
function normalizeFlowchartLabel(text) {
  return normalizeEscapedText(text);
}
function normalizeMindmapLabel(content) {
  return content.replace(/^root\(\(/, "").replace(/\)\)$/, "").replace(/^['"]|['"]$/g, "").replace(/^[^:]+:\s+/, "").trim();
}
...
var flowchartDescriptorPatterns = [
...
var flowchartLinkPattern = /\s*(<-->|o--o|x--x|-->|---|-\.->|-\.\-|==>|===|--o|--x)(?:\|([^|]+)\|)?\s*/g;
export {
  autoLayoutNodes,
  defaultDiagram,
